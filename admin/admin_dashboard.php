...
// Grafik
$total = mysqli_query($conn, "SELECT COUNT(status) AS total FROM tb_survey GROUP BY status ORDER BY status DESC");
$status = mysqli_query($conn, "SELECT status FROM tb_survey GROUP BY status ORDER BY status DESC");
?>
